...
def prompt_pretreatment(query: str, candidates: list):
    ...
    similarity_prompt = "我将向你提供一个查询语句和一个候选语句列表，请你从语义相似和内容相似两个方面综合考虑选出候选语句中与查询语句最为匹配的候选项。\n" \
                        "请将你的输出格式调整为: {\"code\": \"succ\", \"sentence\": \"你选择的匹配语句\"}，" \
                        "如果候选语句中没有可以匹配的项，则将输出格式调整为: {\"code\": \"fail\", \"sentence\": \"\"}，实际上即为我希望你的输出是json格式。\n" \
...
def openai_query(content, apikey):
    ...
    response = openai.ChatCompletion.create(
        ...
        temperature=0.3,  # 控制生成的随机性 0-1越高随机性越强
...
if __name__ == '__main__':
    ...
    # temp_list = ['中国科学技术大学教授','江西赣锋锂业股份有限公司董事长，民建江西省委副主委', '四川省巴中市人民检察院党组成员、副检察长', '四川省凉山州教育和体育局党组成员、副局长', '上海市大学优秀毕业生','同济大学教授','上海交通大学医学院附属瑞金医院人员', '无锡市惠山区洛社镇信息物流园区管理办公室副主任','B612 咔叽市场副总裁']
    #
